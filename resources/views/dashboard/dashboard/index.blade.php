    ...
    <div class="col-md-4">
      <div class="info-box">
        ...
        <div class="info-box-content">
          ...
          <span
            class="info-box-number">@money($total_incomes['total'], setting('general.default_currency'), true)</span>
    ...
    <div class="col-md-4">
      <div class="info-box">
        ...
        <div class="info-box-content">
          ...
          <span
            class="info-box-number">@money($total_expenses['total'], setting('general.default_currency'), true)</span>
    ...
    <div class="col-md-12">
      <div class="nav-tabs-custom">
        <ul class="nav nav-tabs pull-right ui-sortable-handle">
          <li class=""><a href="#monthly-chart" data-toggle="tab"
                          aria-expanded="false">{{ trans('general.monthly') }}</a></li>
          <li class="active"><a href="#daily-chart" data-toggle="tab"
                                aria-expanded="true">{{ trans('general.daily') }}</a></li>
              ...
              <ul class="chart-legend clearfix">
                @foreach ($incomes as $item)
                  <li><i class="fa fa-circle"
                         style="color:{{ $item['color'] }};"></i> {{ $item['amount'] . ' ' . $item['label'] }}</li>
                @endforeach
              </ul>
            </div>
          </div>
            ...
            <table class="table table-striped">
              ...
              @foreach($latest_incomes as $item)
                <tr>
                  ...
                  <td
                    class="text-left">{{ $item->category ? $item->category->name : trans_choice('general.invoices', 2) }}</td>
            ...
            <table class="table table-striped">
              ...
              @foreach($latest_expenses as $item)
                <tr>
                  <td class="text-left">{{ Date::parse($item->paid_at)->format($date_format) }}</td>
                  <td
                    class="text-left">{{ $item->category ? $item->category->name : trans_choice('general.bills', 2) }}</td>
                  <td class="text-right">@money($item->amount, $item->currency_code, true)</td>
                </tr>
              @endforeach
              </tbody>
